...
1. 运行 `beta-live-control-panel`，先看当前 stage、next_action 和下一组命令。
2. 运行 `beta-live-handoff`，确认用户审批点、字段收集范围、secret 边界和 final gate 前命令顺序。
3. 运行 `beta-live-intake-checklist`，确认需要采集的飞书 app、beta 群、open_id、callback 校验字段和本地路径。
4. 运行 `beta-live-prep-dry-run`，确认本地准备链路能跑通。
5. 运行 `init-beta-live-config` 生成本地 `configs/beta.live.toml`，再手工填入真实值；或者用 `beta-live-config-from-env` 从环境变量生成。
6. 运行 `beta-live-config-status`，确认配置存在、已被 `.gitignore` 保护、必要字段已填。
7. 运行 `beta-live-config-review`，确认配置安全、字段完整、敏感字段脱敏。
8. 运行 `beta-validation-guide`，确认是否允许进入真实 beta。
